...
14) Clean up
    <img width="763" height="169" alt="image" src="https://github.com/user-attachments/assets/15fccf09-6e16-4fba-89d1-327159cf07dc" />

I correctly deleted everything.
...
3) EndpointSlice is a Kubernetes resource that stores the actual IP addresses of the Pods that belong to a Service. This is how we can connect a Service to the Pods and list all of their IP's that the service can connect to. The service ClusterIP and EndpointSlice in the lab were demonstrating the fact that the EndpointSlice contained the same Pod IPs that were running, while the Service had a different ClusterIP for entry. I was shown through this how Kubernetes routes traffic from the Service to the correct Pods.

   <img width="1200" height="1600" alt="image" src="https://github.com/user-attachments/assets/78ee60eb-a206-4165-bbfd-5a8f964e93d6" />
